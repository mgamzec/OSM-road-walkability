...
DROP FUNCTION IF EXISTS edge_get_polygon_attr;
CREATE OR REPLACE FUNCTION edge_get_polygon_attr(table_polygon TEXT, polygon_attr TEXT)
RETURNS TABLE(id int, arr_polygon_attr TEXT[], arr_shares float[])
 LANGUAGE plpgsql
AS $function$
DECLARE
	max_id integer := (SELECT max(f.id) FROM edge f);
	...
	WHILE min_border < max_id LOOP 
		...
		(
			...
			(
				...
				WHERE ST_INTERSECTS(p.geom, n.geom) AND ST_LENGTH(p.geom) > 0
...
FROM edge_get_polygon_attr('noise_day_street','noise_level_db');
