...
ELSE NULL END AS landuse_simplified, landuse, p.tourism, p.amenity, p.name, ST_Intersection(p.geom,s.geom) AS geom 
FROM planet_osm_polygon p, bbox s
WHERE landuse IS NOT NULL
AND ST_Intersects(p.geom,s.geom);

INSERT INTO landuse_osm 
SELECT 'transportation' AS landuse_simplified, p.amenity AS landuse, p.amenity, p.leisure, p.name, ST_Intersection(p.geom,s.geom) AS geom 
FROM planet_osm_polygon p, bbox s
WHERE amenity = 'parking' 
AND ST_Intersects(s.geom,p.geom);

INSERT INTO landuse_osm 
SELECT 'community' AS landuse_simplified, p.amenity AS landuse, p.amenity, p.leisure, p.name, ST_Intersection(p.geom,s.geom) AS geom 
FROM planet_osm_polygon p, bbox s
WHERE (amenity = 'hospital' OR amenity = 'school') 
AND ST_Intersects(s.geom,p.geom);

INSERT INTO landuse_osm 
SELECT 'water' AS landuse_simplified, p.leisure AS landuse, p.amenity, p.leisure, p.name, ST_Intersection(p.geom,s.geom) AS geom 
FROM planet_osm_polygon p , bbox s
WHERE leisure = 'swimming_pool' AND ST_Intersects(s.geom,p.geom);

INSERT INTO landuse_osm 
SELECT 'leisure' AS landuse_simplified, p.leisure AS landuse, p.amenity, p.leisure, p.name, ST_Intersection(p.geom,s.geom) as geom 
FROM planet_osm_polygon p , bbox s
...
INSERT INTO landuse_osm 
SELECT 'water' AS landuse_simplified, p."natural" AS landuse,  p.amenity, p.leisure, p.name, ST_Intersection(p.geom,s.geom) as geom 
FROM planet_osm_polygon p , bbox s
WHERE "natural"='water' and ST_Intersects(s.geom,p.geom);

INSERT INTO landuse_osm 
SELECT  'nature' AS landuse_simplified, p."natural" AS landuse, p.amenity, p.leisure, p.name,ST_Intersection(p.geom,s.geom) as geom 
FROM planet_osm_polygon p , bbox s
...
ST_Area(p.geom::geography)::integer as area, 
